...
# 把本地整句模型 data/model/model.qjm（训练仓库 ../train 导出三件套，tools/release/pack-model.sh 打成一个文件）原样上传（覆盖）到 `data` Release。
#
...
# 词库 / 语言模型重生成或模型重训之后重跑一次即可；每次覆盖，不留历史。
set -euo pipefail
...
if ! gh release view data >/dev/null 2>&1; then
  gh release create data --prerelease --title "产品数据（CI 打包用）" \
    --notes "输入法随包的词库 / 语言模型（qingjian-data.tar.gz）与本地整句模型单文件（model.qjm）。滚动覆盖，不是软件版本。由 tools/release/data-bundle.sh 上传。"
else
  gh release edit data --notes "输入法随包的词库 / 语言模型（qingjian-data.tar.gz）与本地整句模型单文件（model.qjm）。滚动覆盖，不是软件版本。由 tools/release/data-bundle.sh 上传。"
fi
gh release upload data "$OUT"/*.tar.gz "$OUT/model.qjm" "$OUT/SHA256SUMS" --clobber
# 早先上传的三件套 tar 已被 model.qjm 取代
gh release delete-asset data qingjian-model.tar.gz --yes 2>/dev/null || true
